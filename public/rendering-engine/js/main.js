...
async function setupScene() {
    ...
    vertSrc = await loadNetworkResourceAsText(
        "shared/resources/shaders/verts/simple.vert"
    );
    fragSrc = await loadNetworkResourceAsText(
        "shared/resources/shaders/frags/simple.frag"
    );
...
function initializeMyObject(vertSource, fragSource, objData, shape) {
    ...
    if (shape.shaderType == "normalmap") {
        ...
        ({ normalTex, diffuseTex, ambientTex, specularTex } =
            createNormalTextures(shape.shader));
    } else {
    ...
    shape.myDrawable = new Drawable(
        shape.shaderProgram,
        bufferMap,
        null,
        rawData.vertices.length / 3
    );
    shape.depthDrawable = new Drawable(
        depthShader,
        {},
        null,
        rawData.vertices.length / 3
    ...
    shape.depthDrawable.uniformLocations = depthShader.getUniformLocations([
        "uLightViewMatrix",
        "uProjectionMatrix",
    ]);
    ...
    shape.myDrawable.uniformSetup = () => {
        ...
        if (shape.shaderType == "image") {
            ...
            shape.shaderType == "cubemap" || shape.shaderType == "dynamicCubemap" ) {
            gl.activeTexture(gl.TEXTURE0);
